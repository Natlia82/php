...
<body>
  ...
  <form action="?c=topic&a=adminlist" method="post">
          ...
          <tbody>
            <?php foreach ($administrator as $adm): ?>
              <tr>
                ...
                <td><a href="?c=topic&a=adminlist&id=<?= $adm[0]?>">Удалить</a></td>
              </tr>
            <?php endforeach; ?>
          </tbody>
        </table>
      ...
      <div class="col s8">
        ...
        foreach ($topics as $quest) :
          ...
          foreach ($question as $qu) :
            if ($qu[5] == $quest[0]) {
              ?>
              <a href="index.php?c=topic&a=update&id=<?php echo $qu[0];?>" class="collection-item col s7 offset-s2"><?php echo $qu[1]." Создан: ".$qu[3].
        ...
        if ($_SESSION["new"])  { ?>
          ...
          <button class="waves-effect waves-teal btn-flat waves-light col s5" type="submit" name="save">Сохранить тему
          </button>
          <button class="waves-effect waves-teal btn-flat waves-light col s5" type="submit" name="cansel">Отмена
          </button>
        <?php  }
        ?>
